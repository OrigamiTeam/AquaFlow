...
const unsigned char simboloAviso [2][100] PROGMEM = {
  {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xE0, 0x70, 0x18, 0x18, 0x18, 0x70, 0xE0, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xE0, 0x78, 0x1E, 0x07, 0x01, 0x00, 0xFC, 0xFC, 0xFC, 0x00, 0x01, 0x07, 0x1E, 0x78, 0xE0, 0x80, 0x00, 0x00, 0x00, 0x00, 0x80, 0xE0, 0x78, 0x1E, 0x07, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE7, 0xE7, 0xE7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x07, 0x1E, 0x78, 0xE0, 0x80, 0x03, 0x07, 0x0E, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0E, 0x07, 0x03},
...
uint8_t avisosQtd = 0;
uint8_t avisosIndice = 0;

uint8_t indiceDisplay = 0;
unsigned long millisAnteriorDisplay = 0;
...
void displayInfo() {
  if (indiceDisplay == 0 && avisosQtd == 0 && avisosIndice == 0) {
    for (uint8_t _i = 0; _i < qtdAvisos; _i++) {
      if (avisos[_i] != 0) {
        avisosQtd = _i + 1;
      }
    ...
    #if DEBUG
    Serial.print(F("avisosQtd: "));
    Serial.println(avisosQtd);
    #endif
  }

  if (indiceDisplay == 0) {
    if (avisosIndice < avisosQtd) {
      boolean _erro = false;
      uint8_t _tipo = avisos[avisosIndice] >> 12;
      if (_tipo >= 0x3) {
        _erro = true;
      }
      ...
      Serial.println(avisos[avisosIndice] & 0xFFF, HEX);
      #endif

      LCD.clear(0, 128);
      delay(1);
      avisoLCD(avisos[avisosIndice] & 0xFFF, _erro, 15);
      avisosIndice++;

      millisAnteriorDisplay = millis();
    }
    else {
      indiceDisplay++;
    }
...
void loop() {
  ...
  if (millisAnteriorDisplay) {
    if (millis() > millisAnteriorDisplay + tempoInfoLCD) {
      indiceDisplay = 5;
      displayInfo();
    }
  }
